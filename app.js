(function() {
  ...
  function setupAdminTransparency() {
    ...
    attachProgressCycle(runBtn);
    attachProgressCycle(copyBtn);
    attachProgressCycle(resetBtn);
    ...
    runBtn.addEventListener('click', function() {
      // Let the progress cycle handle visuals; generate a mock response
      var input = preflight.value.trim();
      ...
      setTimeout(function() {
        setDebug(JSON.stringify(output, null, 2));
        announce('Postflight debug updated.');
      }, 1000); // align with working state
    });

    copyBtn.addEventListener('click', function() {
      var text = debugLog.textContent || '';
      var toCopy = text.length ? text : 'No debug information.';
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(toCopy).then(function() {
          announce('Debug log copied to clipboard.');
        }).catch(function() {
        ...
        try { document.execCommand('copy'); announce('Debug log copied to clipboard.'); }
    ...
    resetBtn.addEventListener('click', function() {
      preflight.value = defaultPreflightJson();
      setDebug('');
      announce('Preflight and debug reset.');
    });
  }
  ...
  function formatCurrency(valueNumber) {
    if (isNaN(valueNumber) || valueNumber === null) return '';
    var n = Number(valueNumber);
  ...
  function setupAddProjectModal() {
    ...
    var todoEl = document.getElementById('tasksTodo');
    var doingEl = document.getElementById('tasksDoing');
    ...
    function close() {
      ...
      if (openBtn) { try { openBtn.focus(); } catch (_) {} }
    }
    ...
    if (monthlyImpactEl) {
      monthlyImpactEl.addEventListener('focus', function() {
        var n = parseCurrency(monthlyImpactEl.value);
        if (n !== null) monthlyImpactEl.value = String(n);
      });
      monthlyImpactEl.addEventListener('blur', function() {
        var n = parseCurrency(monthlyImpactEl.value);
        monthlyImpactEl.value = n === null ? '' : formatCurrency(n);
      });
    }
    ...
    if (imageEl && previewEl) {
      imageEl.addEventListener('change', function() {
        ...
        if (!file) { previewEl.removeAttribute('src'); return; }
        var reader = new FileReader();
        reader.onload = function(ev) { previewEl.src = ev.target && ev.target.result || ''; };
    ...
    if (saveBtn) {
      // Attach progress cycle visuals
      attachProgressCycle(saveBtn);
      saveBtn.addEventListener('click', function() {
        var people = (peopleEl.value || '')
          .split(',')
          .map(function(s) { return s.trim(); })
          .filter(function(s) { return s.length > 0; });
        function extractTasks(textarea) {
        ...
        function persistWithImage(imageDataUrl) {
          ...
          setTimeout(function() {
            if (modal.__editingId) {
              updateProject(modal.__editingId, payload);
            } else {
        ...
        if (previewEl && previewEl.src) {
          persistWithImage(previewEl.src);
        } else if (imageEl && imageEl.files && imageEl.files[0]) {
          var reader = new FileReader();
          reader.onload = function(ev) {
            persistWithImage((ev.target && ev.target.result) || null);
          };
          reader.readAsDataURL(imageEl.files[0]);
        } else {
          persistWithImage(null);
        }
      });
    }
    ...
    if (deleteBtn) {
      ...
      deleteBtn.addEventListener('click', function() {
        ...
        var name = map[modal.__editingId] && map[modal.__editingId].name ? map[modal.__editingId].name : 'this project';
        var ok = confirm('Delete ' + name + '?');
        if (!ok) return;
        setTimeout(function() {
          deleteProject(modal.__editingId);
          renderProjects();
          renderAdminTasks();
          renderHomeGrid();
          renderHomeTasks();
    ...
    // Global Add Task from Admin header
    var addTaskGlobalBtn = document.getElementById('addTaskGlobalBtn');
    if (addTaskGlobalBtn) {
      addTaskGlobalBtn.addEventListener('click', function() {
        var pickerRow = select('taskProjectPickerRow');
        var picker = select('taskProjectPicker');
        if (!pickerRow || !picker) return;
        // Populate projects
        var map = readProjects();
        ...
        var taskModal = select('taskModal');
        if (taskModal) {
          taskModal.__getSelectedProjectId = function() { return picker.value; };
          if (typeof taskModal.openFromGlobal === 'function') {
            taskModal.openFromGlobal();
            return;
          }
        }
        // Fallback
    ...
    modal.populateForEdit = function(id, project) {
      ...
      doingEl.value = (project.tasks && project.tasks.doing || []).join('\n');
      doneEl.value = (project.tasks && project.tasks.done || []).join('\n');
      if (previewEl) {
  ...
  function renderProjects() {
    ...
    ids.forEach(function(id) {
      ...
      var impact = p.monthlyImpact != null ? formatCurrency(p.monthlyImpact) : '';
    ...
    // Row click opens view/edit
    tbody.querySelectorAll('tr.table-row').forEach(function(row, index) {
      row.addEventListener('click', function() {
        var id = ids[index];
        var map = readProjects();
        var modal = document.getElementById('addProjectModal');
        if (modal && map[id] && typeof modal.populateForEdit === 'function') {
          modal.populateForEdit(id, map[id]);
        }
      });
    });
  }

  // Admin tasks aggregation
  function renderAdminTasks() {
    var tTodo = document.getElementById('adminTasksTodo');
    var tDoing = document.getElementById('adminTasksDoing');
    var tDone = document.getElementById('adminTasksDone');
    if (!tTodo || !tDoing || !tDone) return;
    tTodo.innerHTML = '';
    tDoing.innerHTML = '';
    tDone.innerHTML = '';
    ...
    Object.keys(map).forEach(function(id) {
      ...
      function pushRows(list, tbody) {
        (list || []).forEach(function(task) {
          ...
          for (var i = 0; i < stageLists.length; i++) {
            var arr = stageLists[i];
            for (var j = 0; j < arr.length; j++) {
              var e = arr[j];
              var display = e.title || e.short || '';
              if (display === task) { deadline = e.deadline || ''; break; }
            }
          }
          tr.innerHTML = '<td>' + escapeHtml(p.name || '') + '</td>' +
      ...
      var tasks = p.tasks || {};
      pushRows(tasks.todo, tTodo);
      pushRows(tasks.doing, tDoing);
      pushRows(tasks.done, tDone);
    });
  }
  ...
  function renderHomeGrid() {
    ...
    ids.forEach(function(id) {
      ...
      var hasLogo = p.imageDataUrl && typeof p.imageDataUrl === 'string' && p.imageDataUrl.length > 0;
      card.innerHTML = (
        (hasLogo ? '<img class="project-logo" src="' + p.imageDataUrl + '" alt="' + escapeHtml((p.name||'') + ' logo') + '">' : '<div class="project-logo" aria-hidden="true"></div>') +
        '<div class="project-name">' + escapeHtml(p.name || '') + '</div>' +
        '<div class="project-desc">' + escapeHtml(p.type || '') + '</div>'
      );
      card.addEventListener('click', function() {
        var modal = document.getElementById('addProjectModal');
        if (modal && typeof modal.populateForEdit === 'function') {
          modal.populateForEdit(id, p);
        }
      });
  ...
  function renderHomeTasks() {
    var colTodo = document.getElementById('tasksColTodo');
    var colDoing = document.getElementById('tasksColDoing');
    var colDone = document.getElementById('tasksColDone');
    ...
    Object.keys(map).forEach(function(id) {
      var p = map[id];
      function addButtons(tasks, container) {
        (tasks || []).forEach(function(task) {
          var btn = document.createElement('button');
          btn.type = 'button';
          btn.className = 'task-button';
          btn.textContent = (p.name || 'Untitled') + ' | ' + task;
      ...
      var tasks = p.tasks || {};
      addButtons(tasks.todo, colTodo);
      addButtons(tasks.doing, colDoing);
      addButtons(tasks.done, colDone);
    });
  }
  ...
  function defaultPreflightJson() {
    return JSON.stringify({
      action: 'example',
      params: { id: 123, verbose: false }
    }, null, 2);
  }

  function setupTaskModal(openTaskBtn, projectModal) {
    var taskModal = document.getElementById('taskModal');
    var taskTitle = document.getElementById('taskTitle');
    var taskStage = document.getElementById('taskStage');
    var taskShort = document.getElementById('taskShort');
    var taskLong = document.getElementById('taskLong');
    ...
    function clearTaskForm() {
      ...
      if (taskStage) taskStage.value = 'todo';
      if (taskShort) taskShort.value = '';
      if (taskLong) taskLong.value = '';
      if (taskDeadline) taskDeadline.value = '';
    }

    function openTaskModal() {
      taskModal.setAttribute('aria-hidden', 'false');
      var focusables = taskModal.querySelectorAll('button, [href], input, textarea, select, [tabindex]:not([tabindex="-1"])');
      var first = focusables[0];
      var last = focusables[focusables.length - 1];
      trapFocus(taskModal, first, last);
      setTimeout(function() { if (taskTitle) taskTitle.focus(); }, 0);
      document.addEventListener('keydown', onEscape);
    }
    ...
    taskModal.openFromGlobal = function() {
      ...
      var pickerRow = select('taskProjectPickerRow');
      if (pickerRow) pickerRow.style.display = 'block';
      clearTaskForm();
      openTaskModal();
    };
    ...
    if (openTaskBtn) {
      openTaskBtn.addEventListener('click', function() {
        ...
        if (taskModal) taskModal.__getSelectedProjectId = null;
        clearTaskForm();
        openTaskModal();
      });
    }
    closeTaskModalBtn && closeTaskModalBtn.addEventListener('click', function() { closeTaskModal(); });
    cancelTaskBtn && cancelTaskBtn.addEventListener('click', function() { closeTaskModal(); });
    ...
    if (saveTaskBtn) {
      ...
      saveTaskBtn.addEventListener('click', function() {
        ...
        var map = readProjects();
        var proj = map[targetProjectId] || {};
        if (!proj.tasks) proj.tasks = { todo: [], doing: [], done: [] };
        var entry = {
          title: taskTitle.value || '',
          short: taskShort.value || '',
          long: taskLong.value || '',
          deadline: taskDeadline.value || ''
        };
        var stage = (taskStage && taskStage.value) || 'todo';
        var display = entry.title || entry.short || '[Untitled Task]';
        if (!proj.structuredTasks) proj.structuredTasks = { todo: [], doing: [], done: [] };
        proj.structuredTasks[stage].push(entry);
    ...
    function ensureProjectExistsFromForm() {
      var people = (select('projPeople').value || '').split(',').map(function(s){return s.trim();}).filter(Boolean);
      var payload = {
        name: select('projName').value || '',
        description: select('projDesc').value || '',
        individuals: people,
        ...
        structuredTasks: { todo: [], doing: [], done: [] },
        imageDataUrl: (select('projImagePreview') && select('projImagePreview').src) || null
      };
      var id = createProject(payload);
      return id;
    }
  }
